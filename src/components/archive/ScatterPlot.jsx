...
const ScatterPlot = ({
  ...
  useEffect(() => {
    ...
    console.log(`Number of circles: ${circlesSelection.size()}`);
    ...
    if (!selecting) {
      ...
      circlesSelection
        .attr("cursor", "pointer")
        .on("mouseover", (event, d) => handleCircleHover(event, d, g))
        .on("mouseout", (event, d) => handleCircleHover(event, d, g))
      ...
      svg.on(".zoom", null); // Disable zoom behavior but retain the transform
  ...
  const handleCircleHover = (event, d, g) => {
    setHoveredWindow(d);
    const sameLabelCircles = g
      .selectAll(`.${demographics[d.subject].group}`)
      .filter(function (e) {
        return e.subject === d.subject;
      });

    if (event.type === "mouseover") {
      zoomBehaviorRef.current.filter((event) => event.type !== "mousedown"); // Disable zoom on drag (mousedown)

      sameLabelCircles.attr("fill", "rgb(100, 100, 100)");
      setTooltip({
        visible: true,
        x: event.pageX,
        y: event.pageY,
        content: (
          <>
            <p>Group: {demographics[d.subject].group}</p>
    ...
    if (event.type === "mouseout") {
      zoomBehaviorRef.current.filter(() => true); // Re-enable all zoom behavior
      ...
      content: (
        <>
          ...
          <button
            ...
            onClick={() => {
              setSubjectA(d.subject);
              setWindowA(d.window);
              setTooltip2({ ...tooltip, visible: false });
            }}
          >
            Set Subject A
          </button>
  ...
  return (
    <div className="flex">
      <svg ref={svgRef}>
        <g ref={densityPlotRef}>
          {showDensityPlot &&
            zoomTransformRef.current.toString() == zoomIdentity.toString() && (
              <DensityPlot
                data={circles.map((d) => [d.cx, d.cy])}
                width={width}
                height={height}
                margin={margin}
              />
      ...
      <div className="flex justify-center">
        <button
          ...
          style={
            selecting
              ? {
                  backgroundColor: state == 1 ? CLUSTER1_COLOR : CLUSTER2_COLOR,
                }
